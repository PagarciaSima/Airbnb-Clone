...
export class CategoryComponent implements OnInit {
  ...
  private listenNavigationEvents() {
    this.router.events.pipe(
      ...
      .subscribe({
        next: (evt: NavigationEnd) => this.handleNavigationEnd(evt)
      });
  }
  ...
  private listenCategoryQueryParam() {
    this.activatedRoute.queryParams
      .pipe(map(params => params["category"]))
      .subscribe({
        next: (categoryName: CategoryName) => this.handleCategoryChange(categoryName)
      });
  }

  private handleCategoryChange(categoryName: CategoryName) {
